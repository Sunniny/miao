...
const multer = require('multer') // 文件上传请求体解析
const svgCaptcha = require('svg-captcha') // 验证码生成
...
// req是建立这个websocket时的请求对象
wss.on('connection', (ws, req) => { // 这里的req不是express的请求对象，是node的原生请求对象
  console.log('ws connected...')
                                                //  {login: wliejfoi2u3or23, sessionid: weoifjwoeifj}
...
app.get('/vote/:voteId', (req, res, next) => {
  ...
  if (vote) {
    var options = db.prepare('SELECT optionId,content FROM options WHERE voteId = ?').all(voteId)
...
app.post('/vote/:voteId', (req, res, next) => {
  var { voteId } = req.params  // 3
  var { optionIds } = req.body // [5, 8, 9]
